    ...
    <>
      <div className="flex justify-between items-center pb-4">
        <div className="flex gap-4">
          <h1 className="text-vilvBlue text-xl font-semibold pb-4">Inschrijven op trainingen</h1>
          ...
          {isAdmin &&
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <button className="bg-vilvGreen text-white p-2 rounded-md">Training toevoegen</button>
              </DialogTrigger>
        ...
        <TableBody>
          {displayedGames?.map((game) => (
            <TableRow key={game.games.id}>
              ...
              <TableCell onClick={() => router.push(`/games/${game.games.id}`)}>{game.games.pitch}</TableCell>
              <TableCell onClick={() => router.push(`/games/${game.games.id}`)}>{game.games.players}</TableCell>
